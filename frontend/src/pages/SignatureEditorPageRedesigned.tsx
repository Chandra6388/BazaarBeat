...
const SignatureEditorPageRedesigned = () => {
  ...
  const isDarkBackground = () => {
    ...
    if (selectedBackground.background_type === 'color') {
      return selectedBackground.background_value !== '#ffffff' &&
        selectedBackground.background_value !== '#f3f4f6' &&
        selectedBackground.background_value !== '#f0f9ff' &&
        selectedBackground.background_value !== '#f0fdf4' &&
        selectedBackground.background_value !== '#fefce8' &&
        selectedBackground.background_value !== '#fef2f2';
    }
    if (selectedBackground.id.includes('blue') ||
      selectedBackground.id.includes('purple') ||
      selectedBackground.id.includes('dark')) {
      return true;
    }
    return false;
  };
  ...
  const buttonsHtml = selectedButtons.map((button) => {
    ...
    if (button.type == "email") {
      link = `mailto:${button.connect_with}`;
    }
    else if (button.type == "contact_us") {
      link = `tel:${button.connect_with}`;
    }
    return `<td style="padding: 3px;">
                  <a href="${link}"
                    style="display:inline-flex; align-items:center; padding:6px 10px; background-color: ${colorMapping[button?.color]}; border-radius: 10px; color:#fff; text-decoration:none; font-size: 12px; ${button.fontStyle === "italic" ? "font-style: italic;" : ""} ${button.fontStyle === "bold" ? "font-weight: bold;" : ""} ${button.fontStyle === "boldItalic" ? "font-style: italic; font-weight: bold;" : ""}">
                    <img src=${btnsIcons[button.type]} width="15px"
                      style="margin-right: 4px; vertical-align:middle;"> ${button.text}
                  </a>
                </td>`;
  ...
  const socialMediaIcons = {
    ...
    "Twitter": "https://los-static.s3.us-east-1.amazonaws.com/tv/s-icon/ic-twitter.png",
    ...
    "Skype": "https://los-static.s3.us-east-1.amazonaws.com/tv/s-icon/ic-skype.png"

  }


  const socialMediaHtml = selectedSocialMedia.map((item) => {
    console.log("socialMediaIcons[item.type]", item.type)
    return ` <p style="margin: 0px;"><a href=${item.link || '#'} style="text-decoration: none; display: inline-block; margin-right: 8px;">
  ...
  const socialMediaHtml1 = selectedSocialMedia.map((item) => {
    return `<div style="margin-top: 10px;  ">
                  <a href=${item.link || '#'}><img src="${socialMediaIcons[item.type]}" alt="${item.type}" style="width: 20px; height: 20px;"></a>
                </div>`;
  });
  const preventAutoLinking = (text, type) => {
    if (!text) return type;
    if (type === "email") return text.replace(".", "&#8203;.");
    if (type === "url") return text.replace(/\./g, "&#8203;.");
    return text;
  };
  ...
  const templatesArr = [
    {
      ...
      html: `
        <table cellpadding="0" cellspacing="0" style="background: #ffffff; border-collapse: collapse; width:650px">
        <tr>
            ...
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td style="width: 40%; padding:30px 0 0 56px;">
                  <h3
                    style="color: #999999; font-size: 20px; font-weight: 500;line-height: 100%;letter-spacing: 0%;color: #7B7B7B; margin: 0;">${formData?.jobTitle}<span
                      style=" display: inline-block;width: 70px;  height: 4px;background-color: #3FA9F5;margin-left: 5px;  border-radius: 2px; position: relative;  top: -2px;"></span>
                  </h3>
                  <h1 style="font-size: 30px; font-weight: 900;line-height: 100%;margin: 2px 0 8px;color: #000000;">${formData.fullName}</h1>
                  <p style="font-weight: 400; font-size: 12px; color: #999999; margin: 4px 0;"><span
                      style="display: inline-block; min-width: 60px;">Phone:</span><a href="tel:+1680773560"
                      style="color: #1976d2; text-decoration: none;">${formData.phone || "Phone"}</a></p>
                  <p style="font-weight: 400; font-size: 12px; color: #999999; margin: 4px 0;"><span
                      style="display: inline-block; min-width: 60px;">Email:</span><a href="mailto:thomas@acmee.com"
                      style="color: #1976d2; text-decoration: none;">${formData.email || ""}</a></p>
                  <p style="font-weight: 400; font-size: 12px; color: #999999; margin: 4px 0;"><span
                      style="display: inline-block; min-width: 60px;">Website:</span><a href="https://acmee.com"
                      style="color: #1976d2; text-decoration: none;">${formData.website || "website"}</a></p>
                       <p style="font-weight: 400; font-size: 12px; color: #999999; margin: 4px 0;"><span
                      style="display: inline-block; min-width: 60px;">Company:</span><a href="https://acmee.com"
                      style="color: #1976d2; text-decoration: none;"> ${formData?.company || "company name"}</a></p>
                  <p style="font-weight: 400; font-size: 12px; color: #999999; margin: 4px 0;"><span
                      style="display: inline-block; min-width: 60px;">Address:</span><a href="#"
                      style="color: #1976d2; text-decoration: none;">${userDetails?.address || ""} ${userDetails?.country || ""}</a></p>
                  <p style="font-weight: 400; font-size: 12px; color: #999999; margin: 4px 0;"><span
                      style="display: inline-block; min-width: 60px;">NMLS:</span><span style="">61131131</span></p>
                </td>
                <td align="center" style="width: 30%; padding-top: 10px; ">
                  <img
                    src="${formData.headshot_url || userDetails?.profile_img}"
                    width="120" height="120" style="border-radius: 50%; display: block; border: 3px solid #64b5f6;"
                    alt="Wade Warren">
                </td>

                <td style="width: 7%; background: #FFFFFF; border-left: 1px solid #e7e7e7; border-top-left-radius: 24.45px; border-bottom-left-radius: 18px;" align="center">${socialMediaHtml.join('')}</td>
              </tr>
            </table>
          </td>
        </tr>
...
`
    ...
    {
      id: "68105bdc985e207279ecf672",
      html: `
<div style="border: 1px solid #e1d6d6; background-color: white; width: 100%; border-radius: 5px; box-shadow: rgba(0, 0, 0, 0.1) 0px 4px 12px;">
  <table cellpadding="0" cellspacing="0" style="width: 100%; height:300px; font-family: Arial, sans-serif;">
    <tr>
      <td style="padding: 10px; vertical-align: top;">
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td align="center" style="width: 25%; padding: 10px;">
              <img src="${formData.headshot_url || 'https://cdn.jsdelivr.net/gh/faker-js/assets-person-portrait/male/512/41.jpg'}" alt="Profile Image"
                style="border-radius: 50%; width: 80px; height: 80px;" />
            </td>
            <td align="center" style="width: 10%; padding-top: 20px; vertical-align: top;">
              <div>
              ${socialMediaHtml1.join('')}
              </div>
            </td>
            <td style="width: 65%; padding: 10px;">
             <p style="margin: 0; font-size: 20px; font-weight: 800;"><a href="#" style="text-decoration: none; color: #434141;">${formData.fullName}</a></p>
             <p style="margin: 4px 0; color: #3f3c3c; font-size: 17px; font-weight: 700;">${formData.jobTitle || "Job Title"} <span>${formData.company ? "| " + formData.company : ""}</span></p>
             <p style="margin: 4px 0; color: #434141;font-size: 16px; font-weight: 600;"><a href="mailto:${formData.email || ''}" style="text-decoration: none; color: #434141;">✉️ ${formData.email || "email"}</a></p>
             <p style="margin: 4px 0; color: #434141; font-size: 16px; font-weight: 600;"><a href="tel:${formData.phone || ''}" style="text-decoration: none; color: #434141;">📞 ${formData.phone || "Phone"}</a></p>
             <p style="margin: 4px 0; color: #434141;font-size: 16px; font-weight: 600;"><a href="${formData.website || '#'}" style="text-decoration: none; color: #434141;">🌐 ${formData.website || "website"}</a></p>
            </td>
          </tr>
        </table>
      </td>
                                  ...
                                  <div className="p-4 border border-[#112F59] rounded-lg bg-[#020e1f]">
                                    ...
                                    <div>
                                      ...
                                      >
                                        ...
                                        <option value="join_meeting">Join Zoom</option>
                                    ...
                                    <div>
                                      <label className="text-sm text-gray-400 mb-1 block">URL / Phone Number</label>
                                      <input
                                        type="text"
                                        className="w-full bg-[#020e1f] border border-[#112F59] rounded p-2 text-white mb-3"
